namespace AuctionHouse.Models
{
    ...
    public partial class AuctionHouseModel : DbContext
    {
        ...
        public void CheckIfFinished(Guid auction_id)
        {
            Auction a = GetAuction(auction_id);
            if (a.closed == null && a.opened != null && ((DateTime)a.opened).AddSeconds(a.duration) < DateTime.Now)
            {
                Bid lastBid = GetLastBid(auction_id);
                if (lastBid != null)
                {
        ...
        {
            return GetAuction(auction_id).Bids.OrderByDescending(b => b.amount).FirstOrDefault();
        }
        ...
        public IEnumerable<AuctionLastBid> GetAuctionsWithLastBid(int amount, int page, //prvobitna ideja je bila da se ne ucitavaju 
                                                                                        //svi elementi, vec samo prvih 100 (na primer),
                                                                                        //ali to nisam stigao da implementiram, pa se zato
                                                                                        //uvek uzimaju svi elementi i page je uvek 0, a
                                                                                        //amount >= od broja svih elemenata
                                                                  string regex = null,
                                                                  string state = null,
                                                                  float? max_price = null,
                                                                  float? min_price = null,
                                                                  string won = null, string owned = null,
                                                                  //samo korisnik koji je objavio aukciju i administratori mogu da je vide
                                                                  //dok ne bude verifikovana
                                                                  int admin_logged_in = 0)
        {
            ...
            if (admin_logged_in == 0 && owned == null)
            {
                query = query.Where(a => a.state != "READY");
            }
            ...
            foreach(AuctionLastBid a in auctions)
                CheckIfFinished(a.Auction.id);
